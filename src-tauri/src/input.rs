...
const INPUT_PROTOCOL: &str = "mykvm.input.v2";
...
fn input_event_to_command(
    ...
    match event {
        ...
        InputEvent::PreciseScroll {
            delta_x,
            delta_y,
            phase,
            momentum_phase,
        } => Some(InputCommand::PreciseScroll {
            delta_x,
            delta_y,
            phase,
            momentum_phase,
...
fn handle_macos_event(
    ...
    let sent = match event_type {
        ...
        CGEventType::ScrollWheel => {
            ...
            let scroll_event = if continuous {
                InputEvent::PreciseScroll {
                    // The fixed-point fields retain the trackpad's fractional
                    // delta. The old integer AXIS fields quantized it to whole
                    // notches before it ever reached the wire.
                    delta_x: event.get_double_value_field(
                        EventField::SCROLL_WHEEL_EVENT_FIXED_POINT_DELTA_AXIS_2,
                    ),
                    delta_y: event.get_double_value_field(
                        EventField::SCROLL_WHEEL_EVENT_FIXED_POINT_DELTA_AXIS_1,
                    ),
                    phase: macos_scroll_phase(event, MACOS_SCROLL_PHASE_FIELD),
                    momentum_phase: macos_scroll_phase(event, MACOS_MOMENTUM_PHASE_FIELD),
                }
            } else {
                InputEvent::Scroll {
                    delta_x: event
...
#[cfg(target_os = "macos")]
fn inject_precise_scroll(
    delta_x: f64,
    delta_y: f64,
    _phase: GesturePhase,
    _momentum_phase: GesturePhase,
) {
    use core_graphics::{
        event::{CGEvent, CGEventTapLocation, ScrollEventUnit},
        event_source::{CGEventSource, CGEventSourceStateID},
...
#[cfg(target_os = "macos")]
fn inject_swipe(_delta_x: f64, _delta_y: f64, _phase: GesturePhase) {}

#[cfg(target_os = "macos")]
fn inject_pinch(_magnification: f64, _phase: GesturePhase, _x: i32, _y: i32) {}
...
fn inject_precise_scroll(
    ...
    crate::windows_input::inject_precise_scroll(delta_x, delta_y, phase, momentum_phase);
}

#[cfg(target_os = "windows")]
fn inject_swipe(delta_x: f64, delta_y: f64, phase: GesturePhase) {
    crate::windows_input::inject_swipe(delta_x, delta_y, phase);
}
...
mod tests {
    ...
    #[cfg(target_os = "macos")]
    #[test]
    fn macos_gesture_phase_bits_map_to_wire_phases() {
        assert_eq!(gesture_phase_from_nsevent_bits(0), GesturePhase::None);
        assert_eq!(gesture_phase_from_nsevent_bits(1 << 0), GesturePhase::Began);
        assert_eq!(gesture_phase_from_nsevent_bits(1 << 2), GesturePhase::Changed);
        assert_eq!(gesture_phase_from_nsevent_bits(1 << 3), GesturePhase::Ended);
        assert_eq!(
            gesture_phase_from_nsevent_bits(1 << 4),
            GesturePhase::Cancelled
        );
    }
